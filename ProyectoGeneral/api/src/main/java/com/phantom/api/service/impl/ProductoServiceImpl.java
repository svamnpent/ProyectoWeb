...
public class ProductoServiceImpl implements ProductoService {
        ...
        public List<ProductoDTO> buscarPorPalabraClave(String keyword) {
                // Simple implementation using repository's buscarPorPalabraClave with unpaged
                // request
                ...
                return productoRepository.findAllConCategoriasYEtiquetas().stream()
                                .filter(p -> p.getNombre().toLowerCase().contains(keyword.toLowerCase())
                                                || (p.getDescripcion() != null && p.getDescripcion().toLowerCase()
                                                                .contains(keyword.toLowerCase()))
                                                || (p.getMarca() != null && p.getMarca().toLowerCase()
                                                                .contains(keyword.toLowerCase())))
                                .map(this::mapToDTO)
                                .toList();
        }
        ...
        public ProductoDTO crear(ProductoRequestDTO request) {
                Categoria categoria = categoriaRepository.findById(request.getCategoriaId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Categoría no encontrada con el ID: " + request.getCategoriaId()));

                Set<Etiqueta> etiquetas = request.getEtiquetaIds() == null ? Set.of()
                                : etiquetaRepository.findByIdIn(request.getEtiquetaIds()).stream()
        ...
        public void eliminar(Long id) {
                if (!productoRepository.existsById(id)) {
                        throw new ResourceNotFoundException(
                                        "No se puede eliminar. Producto no encontrado con el ID: " + id);
                }
